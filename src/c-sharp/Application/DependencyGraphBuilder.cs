...
public sealed class DependencyGraphBuilder(IReadOnlyList<IDependencyParser> _dependencyParsers, BaseOptions _options, ILogger _logger)
{
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
        ...
        CancellationToken ct)
    {
        ...
        catch (Exception ex) { _logger.LogError($"Error while processing '{item.Value}': {ex}"); }
    }
        ...
        CancellationToken ct)
    {
        ...
        {
            _logger.LogError($"Error while processing '{item.Value}': {ex}");
            return null;
        }
    }
